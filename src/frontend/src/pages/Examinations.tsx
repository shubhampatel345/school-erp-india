...
function ClassTimetableCard({
  ...
  function handleDrop(
    ...
    // Only swap subjects (dates/days stay locked in their positions)
    const newEntries = entries.map((entry) => ({ ...entry }));
    const dragSubject = newEntries[draggingIndex].subject;
    const dragMax = newEntries[draggingIndex].maxMarks;
    const dragMin = newEntries[draggingIndex].minMarks;
    newEntries[draggingIndex].subject = newEntries[dropIndex].subject;
    newEntries[draggingIndex].maxMarks = newEntries[dropIndex].maxMarks;
    newEntries[draggingIndex].minMarks = newEntries[dropIndex].minMarks;
    newEntries[dropIndex].subject = dragSubject;
    newEntries[dropIndex].maxMarks = dragMax;
    newEntries[dropIndex].minMarks = dragMin;
  ...
  function moveRow(index: number, direction: "up" | "down") {
    ...
    // Only swap subjects (dates stay locked)
    const tempSubject = newEntries[index].subject;
    const tempMax = newEntries[index].maxMarks;
    const tempMin = newEntries[index].minMarks;
    newEntries[index].subject = newEntries[targetIndex].subject;
    newEntries[index].maxMarks = newEntries[targetIndex].maxMarks;
    newEntries[index].minMarks = newEntries[targetIndex].minMarks;
    newEntries[targetIndex].subject = tempSubject;
    newEntries[targetIndex].maxMarks = tempMax;
    newEntries[targetIndex].minMarks = tempMin;
    onEntriesChange(className, newEntries);
  }
                ...
                editId === e.id && editForm ? (
                  ...
                  >
                    ...
                    <td
                      ...
                      title="Date locked - cannot be moved"
                    >
                      {e.date}
                    </td>
                    <td
                      className="px-3 py-2 text-yellow-400/70 whitespace-nowrap"
                      title="Day locked - cannot be moved"
                    >
                      {e.day}
                    </td>
                    <td
                      className="px-3 py-2 text-blue-300 font-medium cursor-grab active:cursor-grabbing"
                      draggable
                      onDragStart={(ev) => {
                        ev.stopPropagation();
...
// ── Combined Excel-style timetable view ──────────────────────────────────────
interface CombinedTimetableViewProps {
  generatedByClass: Record<string, ExamTimetableEntry[]>;
  classes: string[];
  examName: string;
}

function CombinedTimetableView({
  generatedByClass,
  classes,
  examName,
}: CombinedTimetableViewProps) {
  // Collect all unique dates across all classes, sorted chronologically
  const allDatesSet = new Set<string>();
  for (const cls of classes) {
    for (const entry of generatedByClass[cls] || []) {
      if (entry.date && entry.date !== "TBD") allDatesSet.add(entry.date);
    }
  }
  ...
  function getSubjectForClass(cls: string, date: string): string {
    return (
      (generatedByClass[cls] || []).find((e) => e.date === date)?.subject || "-"
    );
  }
  ...
  function handlePrintCombined() {
    ...
    const schoolName = profile.schoolName || "School Name";
    const headerCols = ["Date", "Day", ...classes]
      .map(
        (c) =>
          `<th style="padding:6px 10px;background:#1a5276;color:#fff;font-size:11px;text-align:left;border:1px solid #bbb;">${c}</th>`,
      )
      .join("");
    const bodyRows = allDates
      .map((date) => {
        const day = parseDayFromDate(date);
        const subjectCols = classes
          .map(
            (cls) =>
              `<td style="padding:5px 8px;border:1px solid #ddd;font-size:11px;">${getSubjectForClass(cls, date)}</td>`,
          )
          .join("");
        return `<tr><td style="padding:5px 8px;border:1px solid #ddd;font-size:11px;white-space:nowrap;">${date}</td><td style="padding:5px 8px;border:1px solid #ddd;font-size:11px;color:#8B4513;">${day}</td>${subjectCols}</tr>`;
      })
      .join("");
    const win = window.open("", "_blank");
    if (!win) return;
    win.document.write(`<!DOCTYPE html>
<html><head><title>Combined Timetable - ${examName}</title>
<style>* { margin:0; padding:0; box-sizing:border-box; } body { font-family: Arial, sans-serif; padding: 20px; } h1 { text-align:center; font-size:18px; font-weight:bold; } h2 { text-align:center; font-size:14px; color:#1a5276; margin:4px 0 12px; } @media print { body { padding: 10px; } }</style>
</head><body>
<h1>${schoolName}</h1>
<h2>${examName} — Combined Examination Timetable</h2>
<table style="width:100%;border-collapse:collapse;">
<thead><tr>${headerCols}</tr></thead>
<tbody>${bodyRows}</tbody>
</table>
<div style="margin-top:40px;display:flex;justify-content:space-between;">
<div style="text-align:center;width:180px;border-top:1px solid #000;padding-top:4px;font-size:11px;">Class Teacher</div>
<div style="text-align:center;width:180px;border-top:1px solid #000;padding-top:4px;font-size:11px;">Principal</div>
</div>
</body></html>`);
    win.document.close();
    setTimeout(() => win.print(), 400);
  ...
  function handleExportCSV() {
    const header = ["Date", "Day", ...classes].join(",");
    const rows = allDates.map((date) => {
      const day = parseDayFromDate(date);
      const subjectCols = classes.map((cls) => {
        const subj = getSubjectForClass(cls, date);
        return `"${subj.replace(/"/g, '""')}"`;
      });
      return [`"${date}"`, `"${day}"`, ...subjectCols].join(",");
    });
    ...
    a.href = url;
    a.download = `${examName.replace(/[^a-zA-Z0-9]/g, "_")}_timetable.csv`;
    a.click();
    ...
    <div className="bg-gray-800 border border-gray-700 rounded-lg overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-700">
        <div>
          ...
          <p className="text-gray-400 text-[10px] mt-0.5">
            Excel-style view: all classes in a single table
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs px-3 py-1.5 rounded transition"
            data-ocid="exam.combined.print_button"
          >
            🖨 Print Combined
          </button>
          <button
            ...
            className="flex items-center gap-1.5 bg-emerald-700 hover:bg-emerald-600 text-white text-xs px-3 py-1.5 rounded transition"
            data-ocid="exam.combined.secondary_button"
          >
            📥 Export CSV
          </button>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          <thead>
            <tr style={{ background: "#1a1f2e" }}>
              <th className="text-left px-3 py-2 text-gray-300 whitespace-nowrap font-semibold border-r border-gray-700">
                Date
              </th>
              <th className="text-left px-3 py-2 text-gray-300 whitespace-nowrap font-semibold border-r border-gray-700">
                Day
              ...
              {classes.map((cls) => (
                <th
                  key={cls}
                  className="text-left px-3 py-2 text-blue-300 whitespace-nowrap font-semibold border-r border-gray-700"
                >
                  {cls}
                </th>
              ))}
          ...
          <tbody>
            {allDates.map((date, i) => {
              ...
              return (
                ...
                >
                  <td className="px-3 py-2 text-white whitespace-nowrap border-r border-gray-800 font-mono text-[11px]">
                    {date}
                  </td>
                  <td className="px-3 py-2 text-yellow-400 whitespace-nowrap border-r border-gray-800">
                    {day}
                  </td>
                  {classes.map((cls) => {
                    const subj = getSubjectForClass(cls, date);
                    return (
                      <td
                        key={cls}
                        className={`px-3 py-2 border-r border-gray-800 ${subj === "-" ? "text-gray-600" : "text-blue-300 font-medium"}`}
                      >
                        {subj}
                      </td>
                    );
...
export function Examinations() {
  ...
  function handleGenerateTimetable() {
    ...
    for (const cls of wizardClasses) {
      ...
      // Fisher-Yates shuffle a copy of subjects for random distribution each time
      const shuffledSubjects = [...subjects];
      for (let i = shuffledSubjects.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [shuffledSubjects[i], shuffledSubjects[j]] = [
          shuffledSubjects[j],
          shuffledSubjects[i],
        ];
      }
      const classEntries: ExamTimetableEntry[] = [];
      shuffledSubjects.forEach((subj, idx) => {
    ...
    // NOTE: Does NOT save to global timetableEntries. User must click "Save Timetable" explicitly.
  }
  ...
  function handleClassEntriesChange(
    ...
    // NOTE: Does not sync to global timetable until "Save Timetable" is clicked
  }
  ...
  const [savedTimetablesList, setSavedTimetablesList] = useState<
    {
      id: string;
      examName: string;
      savedAt: string;
  ...
  >(() => {
    try {
      return JSON.parse(localStorage.getItem("erp_saved_timetables") || "[]");
    } catch {
      return [];
    }
  });
  const [savedListExpanded, setSavedListExpanded] = useState(false);
  const [saveTimetableNotif, setSaveTimetableNotif] = useState<string | null>(
    null,
  );

  function handleSaveTimetable() {
    // Collect all entries from generatedByClass
    const allNewEntries: ExamTimetableEntry[] = [];
    for (const cls of Object.keys(generatedByClass)) {
      allNewEntries.push(...generatedByClass[cls]);
    }
    if (allNewEntries.length === 0) return;

    // Merge into global timetable (replace entries for same examGroup + class)
    const classesInGenerated = Object.keys(generatedByClass);
    setTimetableEntries((prev) => {
      const filtered = prev.filter(
        (e) =>
          !(
            e.examGroup === wizardExamName &&
            classesInGenerated.includes(e.className)
          ),
      );
    ...
    const record = {
      ...
      classes: classesInGenerated,
      entryCount: allNewEntries.length,
    };
    setSavedTimetablesList((prev) => {
      const updated = [...prev, record];
      localStorage.setItem("erp_saved_timetables", JSON.stringify(updated));
      return updated;
    });

    setSaveTimetableNotif("✅ Timetable saved successfully!");
    setTimeout(() => setSaveTimetableNotif(null), 3000);
  }

  function handleDeleteSavedTimetable(id: string) {
    setSavedTimetablesList((prev) => {
        ...
        <div className="space-y-4">
          ...
          {showGenerated && orderedGeneratedClasses.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-400 text-[10px]">
                    — drag subject cells or use ▲▼ to reorder · dates are locked
                    🔒
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white text-sm font-semibold px-5 py-2 rounded-lg transition shadow-lg shadow-green-900/30"
                    data-ocid="exam.timetable.save_button"
                  >
                    💾 Save Timetable
                  </button>
              ...
              {/* Combined Excel-style view */}
              <CombinedTimetableView
                generatedByClass={generatedByClass}
                classes={orderedGeneratedClasses}
                examName={wizardExamName}
              />

              {/* Save Timetable button (bottom) */}
              <div className="flex justify-end pt-1">
                <button
                  type="button"
                  onClick={handleSaveTimetable}
                  className="flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white text-sm font-semibold px-6 py-2.5 rounded-lg transition shadow-lg shadow-green-900/30"
                  data-ocid="exam.timetable.save_button"
                >
                  💾 Save Timetable
                </button>
              ...
              <div className="bg-gray-800/60 p-4">
                {savedTimetablesList.length === 0 ? (
                  <p
                    ...
                    data-ocid="exam.saved.empty_state"
                  >
                    No timetables saved yet. Generate and save one above.
                  </p>
                ) : (
                  <div className="rounded-lg overflow-hidden border border-gray-700">
                    <table className="w-full text-xs">
                      ...
                      <tbody>
                        {savedTimetablesList.map((t, i) => (
                          <tr
                            key={t.id}
                            style={{
                              background: i % 2 === 0 ? "#111827" : "#0f1117",
                            }}
                            data-ocid={`exam.saved.item.${i + 1}`}
                          >
                            ...
                            <td className="px-3 py-2">
                              <button
                                type="button"
                                onClick={() => handleDeleteSavedTimetable(t.id)}
                                className="text-red-400 hover:text-red-300 text-[11px] flex items-center gap-1 transition"
                                data-ocid="exam.saved.delete_button"
                              >
